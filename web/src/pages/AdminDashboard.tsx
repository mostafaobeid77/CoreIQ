...
export function AdminDashboardPage() {
	...
	useEffect(() => {
		// Poll for pending requests count if superadmin
		if (!isSuperadmin) return;
		const checkPending = async () => {
			try {
				const data = await adminApi.getAdminRequests('pending');
				setPendingRequestsCount(data.requests.length);
			} catch (e) { console.error('Failed to check pending reqs', e) }
		};
		checkPending();
		// Poll every 60s
	...
	const menuItems = [
		...
		// This approach is tricky because we need the stats to know if we should show the badge.
		// The stats are currently loaded inside DashboardHome, but we need them in the parent (AdminDashboardPage) to show the badge in the sidebar for ALL tabs.
		// I will hoist the stats fetching logic or just fetch pending requests count in the parent.
		// Or I can just pass a prop if I restructure, but fetching separately is easier for now to minimize refactor.
		// Let's modify the component structure first in the next tool call, or do a small fetch in useEffect of AdminDashboardPage.
		// Wait, I can't modify the `menuItems` array inside the render dynamically easily without state.
		// I will add a `pendingRequests` state to `AdminDashboardPage`, fetch it, and use it in the menuItems.
			...
			<div className="space-y-6">
				...
				<div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
					...
					<StatCard
						...
						icon={Activity}
						color="amber"
						highlight={stats?.pendingWorkouts > 0}
					/>
					<StatCard
					...
					<header className="mb-8 flex items-center justify-between">
						...
						{activeTab === 'dashboard' && (
							<button
								...
								className="bg-white/5 hover:bg-white/10 text-white border border-white/10 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2"
							>
